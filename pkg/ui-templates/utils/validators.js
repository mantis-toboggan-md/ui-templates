...
export const openAPIV3SchemaValidators = function(
  ...
  if (maximum) {
    if (exclusiveMaximum) {
      out.push((val) => Number(val) >= Number(maximum) ? t('validation.exclusiveMaxValue', { key, maximum }) : undefined
      );
    } else {
      out.push((val) => Number(val) > Number(maximum) ? t('validation.maxValue', { key, max: maximum }) : undefined
      );
    }
  }
  ...
  if (minimum !== undefined) {
    if (exclusiveMinimum) {
      out.push((val) => Number(val) <= Number(minimum) ? t('validation.exclusiveMinValue', { key, minimum }) : undefined
      );
    } else {
      out.push((val) => Number(val) < Number(minimum) ? t('validation.minValue', { key, min: minimum }) : undefined
      );
    }
  }
  if (minLength !== undefined) {
    out.push((val) => val && val.length < Number(minLength) ? t('validation.minLength', { key, min: minLength }) : undefined
    );
  }
  if (maxLength) {
    out.push((val) => val && val.length > Number(maxLength) ? t('validation.maxLength', { key, max: maxLength }) : undefined
    );
  }
  if (maxItems) {
    out.push((val) => val && val.length > maxItems ? t('validation.maxItems', { key, maxItems }) : undefined
    );
  }

  if (minItems !== undefined) {
    out.push((val) => val && val.length < minItems ? t('validation.minItems', { key, minItems }) : undefined
    );
  }
  ...
  if (uniqueItems) {
    out.push((val) => val && val.filter((item, index) => val.indexOf(item) !== index).length ? t('validation.uniqueItems', { key }) : undefined
    );
  }

  if (requiredFields) {
    out.push((val) => requiredFields.find((key) => val[key] === undefined) ? t('validation.requiredFields', {
      key,
...
const regexValidator = function(
  ...
  return (val) => val && !val.match(regexp) ? errorMessage : undefined;
};
...
const stringFormatValidators = function(
  ...
  switch (format) {
  case 'hostname':
    return (val) => formRules.wildcardHostname(val);
  case 'ipv4':
    return (val) => (val && !isIpv4(val) ? errorMessage : undefined);
  case 'cidr':
    return (val) => (val && !isValidCIDR(val)) || val === '' ? errorMessage : undefined;
  case 'mac':
    return (val) => val && !isValidMac(val) ? errorMessage : undefined;
  case 'uuid':
...
export const isDefined = (val) => val || val === false || !isEmpty(val);
...
//       const validBuilds = CP_VERSIONS[type];

//       const parsedVersion = semver.parse(version);

//       if (validBuilds) {
//         return validBuilds.includes(parsedVersion?.build?.[0]) ? '' : t('validation.version');
//       }
...
export const hostValidator = function(t) {
  return stringFormatValidators(t, { key: 'Host' }, 'hostname');
};

export const portValidator = function(t) {
  return (val) => val && isNaN(val) ? t('validation.port') : undefined;
};

export const cidrValidator = function(t) {
  return stringFormatValidators(t, { key: 'Value' }, 'cidr');
};

export const urlValidator = function(t) {
  return (val) => val && !val.match(/^https?:\/\/(.*)$/) ? t('validation.url') : undefined;
};

export const providerVersionValidator = function(t) {
  return (val) => val &&
    !val.match(
      /^v(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$/
    ) ? t('validation.version') : undefined;
};

export const providerNameValidator = function(t) {
  return (val) => val &&
    !val.match(
      /^[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*$/
    ) ? t('validation.name') : undefined;
};
